...
class Editor(QsciScintilla):
    ...
    def show_context_menu(self, pos):
        menu = QMenu(self)
        if self.is_python_file:  # Only enable for Python files
    ...
    def go_to_definition(self):
        ...
        line, index = self.getCursorPosition()  # Get the cursor position
        script = Script(self.text(), path=str(self.path))  # Jedi script
        try:
            definitions = script.goto(line + 1, index)  # Get definitions
            if definitions:
                definition_path = Path(definition.module_path)  # File path of the definition
                definition = definitions[0]
                if definition_path.exists():
                    self.main_window.set_new_tab(definition_path)  # Open file in new tab
                    editor: Editor = self.main_window.tab_view.currentWidget()
                    editor.setCursorPosition(definition.line - 1, definition.column)  # Move cursor
